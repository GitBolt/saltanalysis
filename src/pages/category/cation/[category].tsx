...
const CategoryPage: React.FC<CategoryPageProps> = ({ category, salts }) => {
  return (
    <Layout
      ...
      canonicalUrl={`https://saltanalysis.com/category/cation/${category}`}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const { cations } = getAllCategories();
  const paths = cations.map(category => ({
    params: { category }
  }));
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const salts = getSaltsByCategory(category, 'cation');
